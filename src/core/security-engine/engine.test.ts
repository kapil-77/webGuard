...
describe('SecurityEngine', () => {
  it('produces a clean report for an empty registry', async () => {
    const report = await new SecurityEngine().analyze(snapshot);
  ...
  it('runs detectors in registration order and merges findings', async () => {
    ...
    const report = await new SecurityEngine(registry).analyze(snapshot);
  ...
  it('times out hung detectors', async () => {
    ...
    const report = await new SecurityEngine(registry, { detectorTimeoutMs: 25 }).analyze(snapshot);
